...
class br_ram_data_rd_pipe_random_sequence #(
  ...
  function void fill_random_payload(input BrRamDataRdPipeItem item);
  ...
  function void fill_tile_pattern_payload(input BrRamDataRdPipeItem item,
                                          input int unsigned item_index);
    int tile_width;

    tile_width = br_math::ceil_div(Width, WidthTiles);
    item.word_data = '0;
  ...
  function void fill_walking_one_payload(input BrRamDataRdPipeItem item,
                                         input int unsigned item_index);
    item.word_data = '0;
  ...
  function void fill_item_payload(
      input BrRamDataRdPipeItem item,
  ...
  task fill_random(
    ...
    for (int unsigned i = 0; i < num_items; i++) begin
      ...
      if ((valid_mode == BrRamDataRdPipeValidIdle) && (item.word_data == '0)) begin
